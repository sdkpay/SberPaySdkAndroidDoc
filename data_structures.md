...
### Легенда
- **Устаревший** – параметр считается не актуальным. Он будет работать сейчас, но может быть удален в следующих версиях без предварительного уведомления. Используйте актуальные альтернативы
...
|Параметр|Описание|Устаревший|
|---|---|---|
|Default|Оплата с использованием 6-ти частей и обычной оплатой (без paymentToken и paymentOrder)|Нет|
|WithBankInvoiceId|Оплата с параметрами для проведения оплаты (с paymentToken и paymentOrder)|Да|
|WithBonuses|Оплата со списанием бонусов «Спасибо»|Нет|
|WithoutRefresh|Оплата без рефреша с использованием 6-ти частей и обычной оплатой (без paymentToken и paymentOrder)|Нет|
|WithPaymentAccount|Оплата с использованием платежного счета|Нет|
|WithPartPay|Оплата с использованием платежа на 6 частей|Нет|
|WithPhoneNumber|Оплата с авторизацией только по номеру телефона|Нет|
|BindingPayment|Оплата по связке<br>Требует обязательный параметр `bindingId` — Уникальный идентификатор связки|Нет|
|WithBinding|Оплата по связке<br>Требует обязательный параметр `bindingId` — Уникальный идентификатор связки|Да|
