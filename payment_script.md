...
## ⚠️ Важно
- коллбэк `{ paymentResult -> }` выполняется на фонов потоке. Если вы хотите что-то сделать в UI потоке при получении результата оплаты, то используйте `Handler` либо `runOnUiThread {}`
